...
<head>
    ...
    <script type="text/javascript">
        var debugId;
        var url = "json_dataDebugs.action?nodeState=${nodeState}";
        var url1 = url;
        $(function () {
            jsons(1, 50, url);		//默认为第一页，每页50条数据
            var dataDebug = ${dataDebug};
            if (dataDebug == true) //正在调试，禁止添加调试
        ...
        function edit(id) {  //结束调试
            ...
            $.ajax({
                type: "post",
                dataType: "json",
                url: "showDetailDataDebug.action",
                data: "debugId=" + id,
        ...
        function save_suc(data) {			//添加or修改成功后的返回函数
            ...
            if (flags == 1) {
                ...
                var dataDebug = data.map;
                var debugId = dataDebug.debugId;
                top.success(0, "修改成功");
                var tSort = $("#tr_" + debugId + ">td:eq(0)").html();
                var tHtml = "<td width=5%>" + tSort + "</td>"
                        + "<td width=14%>" + dataDebug.stDt + "</td>"
                        + "<td width=14%>" + dataDebug.endDt + "</td>"
                        + "<td width=6%>" + dataDebug.userName + "</td>"
                        + "<td width=19%>" + dataDebug.debugNote + "</td>"
                        + "<td width=19%>" + dataDebug.debugResult + "</td>"
        ...
        function query_done() {				//提交查询
            ...
            var url_query = "query_dataDebugs.action?nodeState=${nodeState}&queryVersion=" + queryVersion;
        ...
        function del_done() {
            ...
            $.ajax({
                type: "post",
                dataType: "json",
                url: "removeDataDebug?debugId=" + debugId,
...
<div id="detail" class="div_alt">
    <s:include value="showDetailDataDebug.jsp"/>
</div>
